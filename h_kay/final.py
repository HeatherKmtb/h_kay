...
def wilderness(folderin, fileout, folderout, naming='test03_02_'):
    """
    Function to compute q and provide results (csv) and figures (pdf)
...
def h_variation(folderin, fileout, folderout, naming=3, eco_loc=2):
    """
    Function to compute q and provide results (csv) and figures (pdf)
    ...
    #create df for results
    resultsa = pd.DataFrame(columns = ['eco', 'ID', 'qout', 'r_sq', 'deg_free', 
                                       'adj_r2', 'max_h', 'min_h', 'h_var', 
                                       'max_cd', 'min_cd', 'cd_var'])
    #resultsb = pd.DataFrame(columns = ['eco', 'ID', 'qout', 'r_sq', 'deg_free', 'rmse'])
    ...
    for file in fileList:
        ...
        #get additional height and cd values, plus std
        h_max = final['i_h100'].max()
        h_min = final['i_h100'].min()
        cd_max = final['i_cd'].max()
        cd_min = final['i_cd'].min()
        h_var = h_max - h_min
        cd_var = cd_max - cd_min
        ...
        resultsa = resultsa.append({'eco': eco, 'ID': name, 'qout': qout, 
                                    ...
                                    'adj_r2': adj_r2, 'max_h': h_max, 
                                    'min_h': h_min, 'h_var': h_var, 
                                    'max_cd': cd_max, 'min_cd': cd_min, 
                                    'cd_var': cd_var}, ignore_index=True)
        #if deg_free>=60:
            #resultsb = resultsb.append({'eco': name2, 'ID': name, 'qout': qout, 'r_sq': r_sq, 'deg_free': deg_free, 'rmse': rms}, ignore_index=True)        
            #export to excel
        resultsa.to_csv(fileout)
...
def no_zero(folderin, fileout, folderout, naming=3, eco_loc=2):
    """
    Function to compute q and provide results (csv) and figures (pdf)
